                ...
                if length(obj.units{i_probe})>1
                    ...
                    for i_unit_tmp = 1:length(obj.units{i_probe})
                        ...
                        if length(i_yes_trial_correct)>=3 & length(i_no_trial_correct)>=3 
                           ...
                           % keyboard
                           figpath=[all_animals{i_mice},session_date_tmp,'\SingleUnitsFig',num2str(i_probe)];
                           matpath=[all_animals{i_mice},session_date_tmp,'\SingleUnitsMat',num2str(i_probe)];
...
path='C:\Users\XinHao\Desktop\WM_DualCircuit\data\CIBRZC85\2024_01_15\SingleUnitsMat2\';
...
saveas(gcf,'C:\Users\XinHao\Desktop\WM_DualCircuit\data\CIBRZC85\2024_01_15\SingleUnitsFig1\sig_selective_chandepth_map.fig');
...
for jgroup=1:4
    % load PSTH
    if jgroup==1
       obj0=load('C:\Users\XinHao\Desktop\WM_DualCircuit\data\CIBRZC85\2024_01_15\SingleUnitsMat1\SingleUnits_raster_psth.mat');
    elseif jgroup==4
       obj0=load('C:\Users\XinHao\Desktop\WM_DualCircuit\data\CIBRZC85\2024_01_15\SingleUnitsMat2\SingleUnits_raster_psth.mat');
    end
    t=obj0.t; % 
    sampletime=1.3;
    delaytime=1.7;
...
for regionA = 1: 3
    for regionB = regionA+1: 4
        ...
        for i_condition = 1:4 % 四种条件/trial type
            ...
            Cross_correlation{i_condition} = []; % 不是储存一个值而是时间序列
...
ylimitations = {[-1 1], [0 0.005], [0 0.005], [0 3.5], [-0.1 0.9] , [-0.1 0.9]}; % 根据值域适当修改
conditions = {'YesCorrect', 'NoCorrect', 'YesError', 'NoError'};
colors = {'#018A67', '#1868B2', '#F3A332', '#DE582B'}; 
save_path = 'C:\Users\XinHao\Desktop\WM_DualCircuit\data\CIBRZC85\2024_01_15\FCfigures1\';
% subgroups
...
for i = 1:6
    ...
    mid_x = floor(length(mean_data) / 2);
    xline(mid_x, '--k', 'LineWidth', 1.5); % 黑色虚线
