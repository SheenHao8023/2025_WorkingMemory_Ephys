...
n_animals = length(animals_all);

for i_mice = 1:n_animals
        
    % select recording data
    i_str = findstr(animals_all{i_mice},'\');
    data_file_tmp = dir([animals_all{i_mice},'*_behavior.mat']);
    
    data_file = {};
    for i_file = 1:length(data_file_tmp)
        % --------------- load Solo data -------------------
        load([animals_all{i_mice},data_file_tmp(i_file).name]);
        if ~isempty(findstr(obj.sessionNameTag,'recording'))|~isempty(findstr(obj.sessionNameTag,'Recording')) %
            filename_tmp = data_file_tmp(i_file).name;
            i_str = findstr(filename_tmp,'_behavior');
    ...
    for i_file = 1%1:length(data_file)-1
        ...
        if exist([animals_all{i_mice},data_file{i_file}])
            % --------------- process solo data -------------------
            clear obj noise waveform_tmp unit
            load([animals_all{i_mice},data_file{i_file}]);
            
           
            
            % --------------- load recording depth + laser position data -----------------
%             % this m-file contains all the depth + laser position information
            meta_neuronal_opto_behavior_McircuitModule
%             
            % animal name
            i_str = findstr(animals_all{i_mice},'\');
            animal_ID_tmp = animals_all{i_mice}(i_str(end-1)+1:i_str(end)-1);
            
            % session date
            i_str = findstr(data_file{i_file},'_');
            ...
            disp([animals_all{i_mice},data_file{i_file}]);
            

            Path       = [animals_all{i_mice},session_date_tmp,'\'] % wherever you want to search
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % add 0.1 s to pole offset so pole off is the end of sample period
            Pole_Time_tmp(:,2) = Pole_Time_tmp(:,2)+.1;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                 if (unit.manual_quality_score=='1')
                    ...
                    %if unit.cell_type == 1
                        ...
                        N_trials_all(i_unit,:) = [length(i_yes_trial_correct),...
                    ...
                    PSTH1=nan;
                        if length(i_yes_trial_correct)>=3 & length(i_no_trial_correct)>=3 
                           ...
                           for trialtype_tmp=1:4
                            ...
                            [psth1 t PSTH1] = func_getPSTH(spike_times_psth,-3.5,2);
                           ...
                           Spikequality(i_unit,:)=[spikeamp_tmp spikeisi_tmp spikecount spiketrialnum];
                            ...
                            for fk=1:3                                                     
                                switch fk
                                    ...
                                    case 3
                                        if prod(size(PSTH{i_unit,1}))>1&prod(size(PSTH{i_unit,3}))>1
                                        psthA=mean([PSTH{i_unit,1};PSTH{i_unit,3}]);
                                        else if prod(size(PSTH{i_unit,1}))>1&prod(size(PSTH{i_unit,3}))<=1
                                            psthA=Unit{i_unit,1};
                                            else if prod(size(PSTH{i_unit,1}))<=1&prod(size(PSTH{i_unit,3}))>1
                                            psthA=Unit{i_unit,5};
                                                else
                                        ...
                                        if prod(size(PSTH{i_unit,2}))>1&prod(size(PSTH{i_unit,4}))>1
                                        psthB=mean([PSTH{i_unit,2};PSTH{i_unit,4}]);
                                        else if prod(size(PSTH{i_unit,2}))>1&prod(size(PSTH{i_unit,4}))<=1
                                            psthB=Unit{i_unit,3};
                                            else if prod(size(PSTH{i_unit,2}))<=1&prod(size(PSTH{i_unit,4}))>1
                                            psthB=Unit{i_unit,7};
                                                else
...
%                             keyboard
%                             close all
                            figpath=[animals_all{i_mice},session_date_tmp,'\SingleUnitsFig',num2str(i_probe)];
                            matpath=[animals_all{i_mice},session_date_tmp,'\SingleUnitsMat',num2str(i_probe)];
                            tfdir=isdir(figpath);
                            if tfdir==0
                                mkdir(figpath);
                            end
...
%                             saveas(fh,[figpath,'\SingleUnit',num2str(i_unit),'.fig'],'fig');
            ...
            save([matpath,'\SingleUnits_raster_psth.mat'],'Spikequality','Unit','PSTH','Trials','sig_selective','Celltype','FR_pref','N_trials_all','t','unitCh','Depth_all','Pole_Time_tmp','Cue_Time_tmp','-v7.3');
...
path='D:\Data\CIBRZC85\2024_01_15\SingleUnitsMat2\';
load([path,'SingleUnits_raster_psth.mat']);
figure;
...
% load('D:\Data\CIBRZC85\2024_01_15\SingleUnitsMat1\SingleUnits_raster_psth.mat');
% 
...
saveas(gcf,'D:\Data\CIBRZC85\2024_01_15\SingleUnitsFig1\sig_selective_chandepth_map.fig');
%% ALM SC SNr CD corr
CD_proj_all={};
trial_all={};
for jgroup=1:4
    if jgroup==1
       obj0=load(['D:\Data\CIBRZC85\2024_01_15\SingleUnitsMat1\SingleUnits_raster_psth.mat']);
    elseif jgroup==4
       obj0=load(['D:\Data\CIBRZC85\2024_01_15\SingleUnitsMat2\SingleUnits_raster_psth.mat']);
    end
t=obj0.t;
sampletime=1.3;
delaytime=1.7;
...
for j=1:3
    for k=j+1:4
        z=z+1;
        i_yes_trial_correct=intersect(trial_all{j,1},trial_all{k,1});
        i_no_trial_correct=intersect(trial_all{j,2},trial_all{k,2});
        i_yes_trial_error=intersect(trial_all{j,3},trial_all{k,3});
        i_no_trial_error=intersect(trial_all{j,4},trial_all{k,4});

        rcj=mean(CD_proj_all{j,1}(find(t<-0.2&t>-0.6),find(ismember(trial_all{j,1},i_yes_trial_correct))),1);
        lcj=mean(CD_proj_all{j,2}(find(t<-0.2&t>-0.6),find(ismember(trial_all{j,2},i_no_trial_correct))),1);
        rej=mean(CD_proj_all{j,3}(find(t<-0.2&t>-0.6),find(ismember(trial_all{j,3},i_yes_trial_error))),1);
        lej=mean(CD_proj_all{j,4}(find(t<-0.2&t>-0.6),find(ismember(trial_all{j,4},i_no_trial_error))),1);
        
        rck=mean(CD_proj_all{k,1}(find(t<-0.2&t>-0.6),find(ismember(trial_all{k,1},i_yes_trial_correct))),1);
        lck=mean(CD_proj_all{k,2}(find(t<-0.2&t>-0.6),find(ismember(trial_all{k,2},i_no_trial_correct))),1);
        rek=mean(CD_proj_all{k,3}(find(t<-0.2&t>-0.6),find(ismember(trial_all{k,3},i_yes_trial_error))),1);
        lek=mean(CD_proj_all{k,4}(find(t<-0.2&t>-0.6),find(ismember(trial_all{k,4},i_no_trial_error))),1);

        perfr=length(i_yes_trial_correct)/length([i_yes_trial_correct i_yes_trial_error]);
        perfl=length(i_no_trial_correct)/length([i_no_trial_correct i_no_trial_error]);
        subplot(3,3,z);
        scatter(rcj,rck,'bo','filled');hold on;
        scatter(lcj,lck,'ro','filled');hold on;
        scatter(rej,rek,'bo');hold on;
        scatter(lej,lek,'ro');hold on;
        xlabel([region{j},'-CDdelayProj']);
        ylabel([region{k},'-CDdelayProj']);
        title(['perfr:',num2str(perfr),'perfl:',num2str(perfl)]);

        xdata=[rcj lcj rej lej]';
        ydata=[rck lck rek lek]';
        FM = fitlm(xdata,ydata,'linear');
        plot(xdata,FM.Coefficients{2,'Estimate'}*xdata+FM.Coefficients{1,'Estimate'},'k');hold on;
        text(0,0,['R2:',num2str(FM.Rsquared.Ordinary)]);hold on;
        [r,p]=corr(xdata,ydata,'type','Pearson');
        text(0,5,['r:',num2str(r)]);hold on;
        text(0,10,['p:',num2str(p)]);hold on;

% canonical correlation
        prcj=mean(psth_all{j,1}(find(t<-0.2&t>-0.6),find(ismember(trial_all{j,1},i_yes_trial_correct)),:),1);
        plcj=mean(psth_all{j,2}(find(t<-0.2&t>-0.6),find(ismember(trial_all{j,2},i_no_trial_correct)),:),1);
        prej=mean(psth_all{j,3}(find(t<-0.2&t>-0.6),find(ismember(trial_all{j,3},i_yes_trial_error)),:),1);
        plej=mean(psth_all{j,4}(find(t<-0.2&t>-0.6),find(ismember(trial_all{j,4},i_no_trial_error)),:),1);
        
        prck=mean(psth_all{k,1}(find(t<-0.2&t>-0.6),find(ismember(trial_all{k,1},i_yes_trial_correct)),:),1);
        plck=mean(psth_all{k,2}(find(t<-0.2&t>-0.6),find(ismember(trial_all{k,2},i_no_trial_correct)),:),1);
        prek=mean(psth_all{k,3}(find(t<-0.2&t>-0.6),find(ismember(trial_all{k,3},i_yes_trial_error)),:),1);
        plek=mean(psth_all{k,4}(find(t<-0.2&t>-0.6),find(ismember(trial_all{k,4},i_no_trial_error)),:),1);

        prcj=reshape(prcj,size(prcj,1)*size(prcj,2),size(prcj,3));
        plcj=reshape(plcj,size(plcj,1)*size(plcj,2),size(plcj,3));
        prej=reshape(prej,size(prej,1)*size(prej,2),size(prej,3));
        plej=reshape(plej,size(plej,1)*size(plej,2),size(plej,3));

        prck=reshape(prck,size(prck,1)*size(prck,2),size(prck,3));
        plck=reshape(plck,size(plck,1)*size(plck,2),size(plck,3));
        prek=reshape(prek,size(prek,1)*size(prek,2),size(prek,3));
        plek=reshape(plek,size(plek,1)*size(plek,2),size(plek,3));

        X=[prcj;plcj;prej;plej];
        Y=[prck;plck;prek;plek];
%         [A,B,r,U,V,stats] = canoncorr(X,Y);
%         figure;
%         for m=1:9
%         subplot(3,3,m);
%         scatter(U(:,m),V(:,m));hold on;
%         end
% canonical correlation

if (j==1|j==3)&k==4
   cdproj_x=[rcj lcj rej lej]';
   GLM=fitglm(Y,cdproj_x);
   CD_corr_glm=normc(table2array(GLM.Coefficients(2:end,1)));
   ALM_glm_fit=table2array(GLM.Fitted(:,1));
subplot(3,3,6+j);
scatter(rcj,ALM_glm_fit(1:length(rcj)),'bo','filled');hold on;
scatter(lcj,ALM_glm_fit(length(rcj)+1:length([rcj lcj])),'ro','filled');hold on;
scatter(rej,ALM_glm_fit(length([rcj lcj])+1:length([rcj lcj rej])),'bo');hold on;
scatter(lej,ALM_glm_fit(length([rcj lcj rej])+1:end),'ro');hold on;
xlabel([region{j},'-CDdelayProj']);
ylabel([region{k},'-glmCDdelayProj']);
xdata=cdproj_x;
ydata=ALM_glm_fit;
FM = fitlm(xdata,ydata,'linear');
plot(xdata,FM.Coefficients{2,'Estimate'}*xdata+FM.Coefficients{1,'Estimate'},'k');hold on;
...
text(0,10,['p:',num2str(p)]);hold on;
if j==1
   CD_corr_glm_SC=CD_corr_glm;
else
   CD_corr_glm_SNr=CD_corr_glm;
end
end
    end
end
CD_corr_glm_SCSNr_angle= acos(dot(CD_corr_glm_SC,CD_corr_glm_SNr)/(norm(CD_corr_glm_SC)*norm(CD_corr_glm_SNr)))
subplot(3,3,8);
plot([1 2],[1 1]);hold on;
plot([1 1+cos(CD_corr_glm_SCSNr_angle)],[1 1+sin(CD_corr_glm_SCSNr_angle)]);hold on;
text(1,1,['CD-corr-glm-SCSNr-angle=',num2str(CD_corr_glm_SCSNr_angle/(2*pi)*360)]);
saveas(gcf,'D:\Data\CIBRZC85\2024_01_15\cross_region_CDproj_corr.fig');
